...
安全上下文:
	1.进程是以某用户的身份运行，进程是发起此进程用户的代理，一次以此用户的身份和权限完成所有操作
	2.权限匹配模型
		判断进程的属主，是否被访问的文件属主，是则应用属主权限
		判断进程的属组，是否被访问的文件属组，是则应用属组权限
		应有other权限
		
SUID
	默认情况下: 用户发起的进程，进程的属主是其发起者，因此，以发起者的身份在运行
...
STICKY
	功用: 对于属组或全局可写的目录，组内的所有用户或系统上的所有用户对此目录中都能创建新文件或删除所有的已有文件，如果此目录有STICKY权限，则每个用户能新建新文件，且只能删除自己的文件
